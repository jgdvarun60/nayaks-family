...
public interface PersonRepository extends JpaRepository<Person, Long> {
    ...
    @Query(value = "select person from Person person left join fetch person.family", countQuery = "select count(person) from Person person")
    Page<Person> findAllWithToOneRelationships(Pageable pageable);

    @Query("select person from Person person left join fetch person.family")
    List<Person> findAllWithToOneRelationships();

    @Query("select person from Person person left join fetch person.family where person.id =:id")
